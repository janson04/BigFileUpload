<template>
  <div>
    ...
    />
    <el-button id="browse_button" type="primary">選擇圖片</el-button>
    <br/>
    <br/>
    <el-tag type="info">圖片預覽區域</el-tag>
        ...
        <el-card :body-style="{ padding: '0px' }">
          ...
          <div style="padding: 5px; display: flex">
              <div style="flex: 3; display: flex; justify-content:center;align-items:center;">
                <span v-if="image.file.status === 1" style="color: aqua;">準備上傳</span>
                <span v-if="image.file.status === 4" style="color: brown">上傳失敗</span>
                <span v-if="image.file.status === 5" style="color: chartreuse">已上傳</span>
                <el-progress v-if="image.file.status === 2" :text-inside="true" :stroke-width="20" :percentage="image.file.percent"></el-progress>
              </div>
              <el-button style="flex: 1" type="text" class="button" @click="deleteFile(index,image.file)">刪除</el-button>
          </div>
        </el-card>
      </el-col>
    </el-row>
